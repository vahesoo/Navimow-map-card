/*
 * Navimow Map Card  (v0.1.1-dev — auto entities, history view, overlay rotation, session filtering, visual editor)
 *
...
const NAVIMOW_MAP_CARD_BASE_URL = (() => {
  // The card may be installed by HACS under:
  // /config/www/community/<repo>/dist/navimow-map-card.js
  // and served in Home Assistant as:
  // /hacsfiles/<repo>/navimow-map-card.js
  //
  // Keep icon paths relative to the actual loaded JS file so bundled icons
  // resolve correctly from icons/<model>.svg or dist/icons/<model>.svg depending on the install layout.
  try {
  ...
  try {
    ...
    const self = scripts.find((s) => (s.src || '').includes('navimow-map-card.js'));
    if (self && self.src) return new URL('.', self.src).href;
  } catch (e) {}

  // Fallback for manual HACS installs. The capitalized repository folder is
  // intentional because GitHub/HACS may preserve the repository name casing.
  return '/hacsfiles/Navimow-map-card/';
})();
